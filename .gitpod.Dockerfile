...
RUN curl -L https://sourceforge.net/projects/cdrtools/files/alpha/cdrtools-3.02a09.tar.bz2/download | tar -xjf- && cd cdrtools-3.02 && make -j`nproc` && sudo install -m 755 mkisofs/OBJ/x86_64-linux-cc/mkisofs /usr/local/bin/mkisofs