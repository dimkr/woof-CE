...
if [ "$USR_SYMLINKS" = "yes" -a ! -e ${TARBALL} ]; then
	$debootstrap --arch=$ARCH --variant=minbase --include=sysvinit-core --make-tarball=${TARBALL} ${DISTRO_COMPAT_VERSION} bdrv ${MIRROR}
elif [ ! -e ${TARBALL} ]; then
	$debootstrap --no-merged-usr --arch=$ARCH --variant=minbase --include=sysvinit-core --make-tarball=${TARBALL} ${DISTRO_COMPAT_VERSION} bdrv ${MIRROR}
fi
