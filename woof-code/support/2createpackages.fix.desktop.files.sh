...
CATDONE='no'
if [ -f rootfs-packages/ppm/usr/local/petget/categories.dat ];then #precaution, but it will be there -- yes, have added code above makes sure it is.
	...
	if [ "$NUMDESKFILE" = "1" ];then
		...
		CATVARIABLE="$(grep -i " ${xNAMEONLY} " rootfs-packages/ppm/usr/local/petget/categories.dat | grep '^PKGCAT' | head -n 1 | cut -f 1 -d '=' | cut -f 2,3 -d '_' | tr '_' '-')" #ex: PKGCAT_Graphic_camera=" gphoto2 gtkam "
